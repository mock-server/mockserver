...
echo "$GOOGLE_CHROME --user-data-dir=$USER_DATA_DIR --no-default-browser-check --no-first-run --disable-default-apps --disable-web-security --enable-potentially-annoying-security-features --enable-extensions http://localhost:$NODE_PORT/SpecRunner.html"
"$GOOGLE_CHROME" --user-data-dir=$USER_DATA_DIR --no-default-browser-check --no-first-run --disable-default-apps --disable-web-security --enable-potentially-annoying-security-features --enable-extensions "http://localhost:$NODE_PORT/SpecRunner.html" &
CHROME_PID=$!
