...
To run as Embedded Jetty run the following command:
    ...
    java -jar <path to mockserver-jetty-1.9-SNAPSHOT-jar-with-dependencies.jar> <port>
...
To run as Vert.X module run the following command:
    ...
    vertx install org.mock-server~mockserver-vertx~1.9-SNAPSHOT
    vertx runmod org.mock-server~mockserver-vertx~1.9-SNAPSHOT
...
Alternatively build from source and run the Vert.X module as a zip file:
    ...
    cd mockserver
    mvn clean package
    vertx runzip mockserver-vertx/target/org.mock-server~mockserver-vertx~1.9-SNAPSHOT.zip

**Web Archive (WAR)**

To run as a WAR deployed on any JEE web server:

    deploy mockserver-war-1.9-SNAPSHOT.jar to web server
...
This will produce a jar file under the target directory called, as follows:
    ...
    mockserver-jetty/target/mockserver-jetty-1.9-SNAPSHOT-jar-with-dependencies.jar
...
For example to run the MockServer on port 9999:
    ...
    java -jar mockserver-jetty/target/mockserver-jetty-1.9-SNAPSHOT-jar-with-dependencies.jar 9999
