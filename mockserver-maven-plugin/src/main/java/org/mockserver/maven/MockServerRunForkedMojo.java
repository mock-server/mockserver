 ...
 *
 *    mvn -Dmockserver.serverPort="1080" -Dmockserver.proxyPort="1090" -Dmockserver.logLevel="TRACE" org.mock-server:mockserver-maven-plugin:3.10.4:runForked
 *
 * @author jamesdbloom
 */
...
//            arguments.add("-agentlib:jdwp=transport=dt_socket,server=y,suspend=y,address=5010");
    ...
    String getVersion() {
        ...
        try {
            ...
            if (is != null) {
                p.load(is);
                version = p.getProperty("version", "3.10.4");
            }
        } catch (Exception e) {
            // ignore
        }
